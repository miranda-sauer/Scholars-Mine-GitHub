...
def email_search(name):
    dct = filterName(name)
    tmp = authority_cursor.execute('''
        SELECT email FROM faculty WHERE 
            (last_name COLLATE NOCASE) = :last 
            AND                
            (first_name COLLATE NOCASE) = :first 
            AND 
            (middle_name COLLATE NOCASE) = :middle
        ''', dct).fetchall() # COLLATE NOCASE removes case sensitivity.
    if len(tmp) == 1:
...
def recordAuthor(srow,scol,name):
    # Write contents of authorDict entry to the output spreadsheet
...
def dictPrint(dct):
    # Pretty print for author dictionary.
